	...
	data pregs (keep=patient_deid idxpren dt_indexprenatal dt_ltfu dt_preg: prenonly outconly dt_ga: preg_:
   	...
   	set Out.pregnancy_&encw._&alg. (rename=(preg_outcome = preg_outcome_crude));
		...
		/*For determining gestastional age codes, we want either the preg outcome date or the date ltfu*/
      	...
      	preg_outcome = preg_outcome_clean;
    ...
    **********************************************************************************************************;
    ...
    proc sql; *all GA encounters for pregnancy flagged;
     	create table AnyGAEnc as
      	select distinct a.patient_deid, a.idxpren, DT_INDEXPRENATAL, prenonly, outconly, dt_gapreg, a.preg_outcome,  
            	...
            	/*added for 1c and 2b code hierarchy ranking*/
             	Case 
                 	/* Apply GA hierarchy in 1c to delivery pregnancy outcomes */
                  	when a.preg_outcome  in ('LBM' 'LBS' 'MLS' 'UDL' 'SB' ) And code_hierarchy='Specific gestational age' then 1  
                  	when a.preg_outcome  in ('LBM' 'LBS' 'MLS' 'UDL' 'SB' ) And code_hierarchy='Extreme prematurity' then 2 
                  	when a.preg_outcome  in ('LBM' 'LBS' 'MLS' 'UDL' 'SB' ) And code_hierarchy in ('Other preterm' 'Post-term') then 3 

                  	/* Apply GA hierarchy in 1c to Abortion pregnancy outcomes */
                  	when a.preg_outcome in ('IAB' 'UAB' 'SAB'  ) And code_hierarchy='Specific gestational age' then 1  
                  	when a.preg_outcome in ('IAB' 'UAB' 'SAB'  ) And code_hierarchy='Abortion' then 2   

                  	/* Apply GA hierarchy in 1c to EM outcomes */
                  	when a.preg_outcome  in ('EM' ) And code_hierarchy='Specific gestational age' then 1  
                  	when a.preg_outcome  in ('EM' ) And code_hierarchy='Extreme prematurity' then 2 
                  	when a.preg_outcome  in ('EM' ) And code_hierarchy in ('Other preterm') then 3 

                  	/* Apply GA hierarchy in 1c to Missing/Unknown outcomes */
                  	when a.preg_outcome  in ('UNK') And code_hierarchy='Specific gestational age' then 1  
                  	when a.preg_outcome  in ('UNK') And code_hierarchy='Extreme prematurity' then 2 
                  	when a.preg_outcome  in ('UNK') And code_hierarchy in ('Other preterm' 'Post-term') then 3  
                  	when a.preg_outcome  in ('UNK') And code_hierarchy in ('Missing') then 4  
     	...
     	create table AnyGA_EstLMPs as 
      	...
      	from 
         	( /*date level totals - want to get counts that meet criteria*/
          		select patient_deid, idxpren, dt_gapreg, preg_outcome, GA_hierarchy, gaoutcmatch, gadaysmatch, dt_LMP , 
...
/*                	count(distinct Case when not (GAOutcMatch and GADaysMatch) then code end) as Count_GACodes_notok*/
         		from anygaenc 
           		group by patient_deid,idxpren, preg_outcome,gaoutcmatch,gadaysmatch, GA_hierarchy , dt_LMP 
         	) a 
      	full join
    ...
    **********************************************************************************************************;
    ...
    proc sql; *apply 1b.1 - one GA code with 1 LMP date estimate;
     	create table GaEnc_1b1 as
      	select distinct a.patient_deid, a.idxpren, dt_gapreg, a.preg_outcome, dt_LMP , days_pregtogaenc_ClosestAbs,
    ...
    ******************************************************************************************/ 
    ...
    proc sql;
     	...
     	create table gaenc_1c2_ptA as
      	select distinct patient_deid,idxpren, dt_gapreg, preg_outcome,
    	...
    	;*take away LMP_AlgStep because this is for all 1c2 folks (down below);
     	...
     	create table gaenc_1c2_ptB3 as
      	select distinct patient_deid,idxpren, dt_gapreg, preg_outcome, pregcomplexpath, 
    ...
    ***************************************************************************************************;
    ...
    proc sql;
        ...
        where b.LMP_AlgBased = 0
           AND a.Preg_outcome='UNK'   /*missing/unknown (prenonly) pregnancies */
     	...
     	create table pregs_2c_zhu_test1st as 
      	...
      	From
	       (select * 
	          from
	           (
	           select distinct a.patient_deid,a.idxpren, dt_gapreg,preg_outcome,  dt_LMP , gestational_age_days,
